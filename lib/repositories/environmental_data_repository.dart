...
class EnvironmentalDataRepository {
  ...
  Future<List<EnvironmentalDataModel>> syncEnvironmentalData({
    ...
    if (kIsWeb) {
      return await _environmentalDataService.fetchEnvironmentalData();
    } else {
      if (isOnline) {
        try {
          ...
          if (localNewData.isNotEmpty) {
            await _environmentalDataService.uploadEnvironmentalData(
              localNewData,
            );
  ...
  Future<void> add(
    ...
    if (kIsWeb) {
      ...
      if (isOnline) {
        try {
          await _environmentalDataService.addEnvironmentalData(
            environmentalData,
          );
